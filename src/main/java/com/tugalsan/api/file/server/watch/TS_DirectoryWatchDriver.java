...
public class TS_DirectoryWatchDriver {
    ...
    private TS_DirectoryWatchDriver(Path dir, TGS_CallableType1_Run<Path> forFile, boolean recursive, TS_FileWatchUtils.Triggers... triggers) throws IOException {
    ...
    public static TS_DirectoryWatchDriver of(Path dir, TGS_CallableType1_Run<Path> forFile, TS_FileWatchUtils.Triggers... triggers) {
        return TGS_UnSafe.call(() -> new TS_DirectoryWatchDriver(dir, forFile, false, triggers));
    }

    @Deprecated //DOUBLE NOTIFY? AND PATH AS FILENAME?
    public static TS_DirectoryWatchDriver ofRecursive(Path dir, TGS_CallableType1_Run<Path> forFile, TS_FileWatchUtils.Triggers... triggers) {
        return TGS_UnSafe.call(() -> new TS_DirectoryWatchDriver(dir, forFile, true, triggers));
    }

    @Deprecated //PATH AS FILENAME?
    public static TS_DirectoryWatchDriver ofFile(Path file, TGS_CallableType0_Run exe) {
    ...
    private void processEvents(TGS_CallableType1_Run<Path> forFile) {
        while (true) {
            WatchKey key;
            try {
